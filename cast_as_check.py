...
def retic_cast(val, src, trg, msg):
    ...
    assert retic_has_type(val, trg), "%s at line %d (expected %s, value of kind %s)" % (msg, inspect.currentframe().f_back.f_lineno, trg, val)
    return val

def retic_check(val, trg, msg):
    assert retic_has_type(val, trg), "%s %s at line %d" % (dir(val), msg, inspect.currentframe().f_back.f_lineno)
    return val
