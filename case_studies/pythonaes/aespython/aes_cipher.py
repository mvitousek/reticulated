...
class AESCipher:
    ...
    def __init__ (self, expanded_key : List(Int)):
        #Store epanded key
    ...
    def _i_sub_bytes (self, state : List(Int)):
        #Run state through inverted sbox
        for i,s in enumerate(state):state[i]=aes_tables.i_sbox[s]

    def _shift_row (self, row : List(Int), shift : Int) -> List(Int):
        #Circular shift row left by shift amount
        row+=row[:shift]
        del row[:shift]
        return row

    def _i_shift_row (self, row : List(Int), shift : Int) -> List(Int):
        #Circular shift row left by shift amount
        row+=row[:shift]
        del row[:4+shift]
        return row
    ...
    def _i_shift_rows (self, state : List(Int)):
        #Extract rows as every 4th item starting at [1..3]
        #Replace row with inverse shift_row operation
        for i in 1,2,3:
            state[i::4] = self._i_shift_row(state[i::4],-i)

    def _mix_column (self, column : List(Int), inverse : Bool) -> List(Int):
    ...
    def cipher_block (self, state : List(Int)) -> List(Int):
    ...
    def decipher_block (self, state : List(Int)) -> List(Int):
